...
def assert_weighted_median(actual_weighted_median, data, weights):
    left_sum = sum([weights[i] for i, x in enumerate(data) if x < actual_weighted_median])
    right_sum = sum([weights[i] for i, x in enumerate(data) if x > actual_weighted_median])
...
class Solutions09Test(TestCase):
    ...
    def test_two_arrays_median(self):
        ...
        expected_median = sorted(data1 + data2)[n - 1]
    ...
    def test_post_office_manhattan(self):
        ...
        post_office_distance_sum = get_weighted_distance_sum(actual_post_office, data, weights)
        for point in data:
            point_distance_sum = get_weighted_distance_sum(point, data, weights)
