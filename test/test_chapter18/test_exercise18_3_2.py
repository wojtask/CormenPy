...
class TestExercise18_3_2(TestCase):
    ...
    def test_b_tree_delete(self):
        ...
        assert_that(T.root.key.elements[:5], contains_exactly('E', 'L', 'P', 'T', 'X'))
        assert_that(T.root.c[1].n, is_(equal_to(2)))
        assert_that(T.root.c[1].key.elements[:2], contains_exactly('A', 'C'))
        assert_that(T.root.c[2].n, is_(equal_to(2)))
        assert_that(T.root.c[2].key.elements[:2], contains_exactly('J', 'K'))
        assert_that(T.root.c[3].n, is_(equal_to(2)))
        assert_that(T.root.c[3].key.elements[:2], contains_exactly('N', 'O'))
        assert_that(T.root.c[4].n, is_(equal_to(3)))
        assert_that(T.root.c[4].key.elements[:3], contains_exactly('Q', 'R', 'S'))
        assert_that(T.root.c[5].n, is_(equal_to(2)))
        assert_that(T.root.c[5].key.elements[:2], contains_exactly('U', 'V'))
        assert_that(T.root.c[6].n, is_(equal_to(2)))
        assert_that(T.root.c[6].key.elements[:2], contains_exactly('Y', 'Z'))
