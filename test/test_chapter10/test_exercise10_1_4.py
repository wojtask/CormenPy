...
from chapter10.exercise10_1_4 import dequeue_, queue_empty, enqueue_, queue_full
...
class TestExercise10_1_4(TestCase):
    ...
    def test_queue_full(self):
        ...
        actual_full = queue_full(queue)

        assert_that(queue.is_modified(), is_(False))
        if queue.head == queue.tail + 1:
            assert_that(actual_full, is_(True))
        elif queue.head == 1 and queue.tail == queue.length:
            assert_that(actual_full, is_(True))
        else:
            assert_that(actual_full, is_(False))
