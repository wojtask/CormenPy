...
def _create_arrays():
    ...
    for i in between(0, k - 1):
        if random.randint(0, 1) == 0:
            ...
            elements = sorted([random.randint(0, 2 ** (k - 1)) for _ in range(2 ** i)])
...
class TestProblem17_2(TestCase):
    ...
    def test_dynamic_binary_insert(self):
        arrays, k = _create_arrays()
        element = random.randint(0, 2 ** (k - 1))
        elements_before = [x for arr in arrays for x in arr]
        # make sure there is a place for the new element
        if len(elements_before) == 2 ** k - 1:
            elements_before.remove(arrays[0][1])
            arrays[0] = Array([])

        dynamic_binary_insert(arrays, element)

        actual_elements_after = [x for arr in arrays for x in arr]
        ...
        assert_that(len(actual_elements_after), is_(equal_to(len(expected_elements_after))))
        for i in between(0, k - 1):
    ...
    def test_dynamic_binary_delete(self):
        ...
        actual_elements_after = [x for arr in arrays for x in arr]
        expected_elements_after = list(elements_before)
