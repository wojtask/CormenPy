...
def fractional_knapsack_bruteforce(w, v, W, i=1):
    if i == w.length:
        return part_item_value(min(w[i], W), w[i], v[i])
    value_no_item = fractional_knapsack_bruteforce(w, v, W, i + 1)
    value_half_item = part_item_value(min(w[i] // 2, W), w[i], v[i]) + \
                      fractional_knapsack_bruteforce(w, v, W - min(w[i] // 2, W), i + 1)
    value_full_item = part_item_value(min(w[i], W), w[i], v[i]) + \
                      fractional_knapsack_bruteforce(w, v, W - min(w[i], W), i + 1)
...
class TestTextbook16_2(TestCase):
    ...
    def test_fractional_knapsack(self):
        ...
        expected_knapsack_value = fractional_knapsack_bruteforce(weights, values, max_weight)
        assert_that(expected_knapsack_value, is_(close_to(actual_knapsack_value, 1e-7)))
